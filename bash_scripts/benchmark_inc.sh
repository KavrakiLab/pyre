...
do 
     ...
     roslaunch --wait pyre benchmark.launch start:=$start end:=$end dataset:=$dset database:=$dbSPARK postfix:=_$db algo:=1 time:=60 &
     roslaunch --wait pyre benchmark.launch start:=$start end:=$end dataset:=$dset database:=$dbFLAME postfix:=_$db algo:=2 time:=60 &
wait
done
